// elm-watch hot {"version":"1.2.6","targetName":"Main","webSocketPort":45653,"webSocketToken":"58a76f4b-af4c-4e52-b041-91b8c725ed4a"}
"use strict";
(() => {
...
${indent.repeat(level)}}`;
  ...
  var VERSION = "1.2.6";
  var WEBSOCKET_TOKEN = "58a76f4b-af4c-4e52-b041-91b8c725ed4a";
...
var $author$project$Main$init = _Utils_Tuple2(
	{customPrimary: '#FFFFFF', customQuart: '#555555', customSecondary: '#CCCCCC', customStroke: '#000000', customTernary: '#999999', selectedTess: 'Hexagonal', selectedTheme: 'Forest'},
...
var $author$project$Polygon$equals = F2(
	function (p1, p2) {
		return _Utils_eq(p1.lengths, p2.lengths) && (_Utils_eq(p1.angles, p2.angles) && _Utils_eq(p1.rotation, p2.rotation));
	});
var $author$project$Rules$eq = F2(
	function (p1, p2) {
		return A2($author$project$Polygon$equals, p1.poly, p2.poly) && _Utils_eq(p1.col, p2.col);
	});
var $author$project$Rules$eq2 = F2(
	function (p1, p2) {
		return _Utils_eq(p1.poly.lengths, p2.poly.lengths) && (_Utils_eq(p1.poly.angles, p2.poly.angles) && _Utils_eq(p1.col, p2.col));
	});
var $author$project$Rules$applies = F2(
	function (rule, p) {
		return rule.rotatable ? A2($author$project$Rules$eq2, rule.anchor, p) : A2($author$project$Rules$eq, rule.anchor, p);
	});
var $elm$core$Basics$sqrt = _Basics_sqrt;
var $author$project$Util$distance = F2(
	function (p1, p2) {
		var dy = p2.y - p1.y;
		var dx = p2.x - p1.x;
		return $elm$core$Basics$sqrt((dy * dy) + (dx * dx));
	});
var $author$project$Util$epsilon = 0.001;
var $author$project$Rules$collides = F2(
	function (p1, p2) {
		return _Utils_cmp(
...
var $author$project$Util$inside = F2(
	function (p1, _v0) {
		var p2 = _v0.a;
		var p3 = _v0.b;
		return (_Utils_cmp(p1.x, p2.x) > -1) && ((_Utils_cmp(p1.x, p3.x) < 1) && ((_Utils_cmp(p1.y, p2.y) > -1) && (_Utils_cmp(p1.y, p3.y) < 1)));
	});
var $elm$core$Basics$negate = function (n) {
	return -n;
};
var $author$project$Util$neg = function (_v0) {
	var x = _v0.x;
	var y = _v0.y;
	return {x: -x, y: -y};
};
...
var $author$project$Util$rotateAround = F3(
	function (origin, angle, p) {
		var sina = $elm$core$Basics$sin(
			$elm$core$Basics$degrees(-angle));
		var p2 = A2($author$project$Util$sub, p, origin);
		var cosa = $elm$core$Basics$cos(
			$elm$core$Basics$degrees(-angle));
		var p3 = {x: (p2.x * cosa) - (p2.y * sina), y: (p2.y * cosa) + (p2.x * sina)};
		return A2($author$project$Util$add, p3, origin);
	});
...
var $author$project$Rules$rt = F3(
	function (origin, angle, pc) {
		return _Utils_update(
			pc,
			{
				centre: A3($author$project$Util$rotateAround, origin, angle, pc.centre),
				poly: A2(
					$author$project$Polygon$setOrigin,
					A3($author$project$Util$rotateAround, origin, angle, pc.poly.origin),
					A2($author$project$Polygon$setRotation, angle + pc.poly.rotation, pc.poly))
			});
	});
var $author$project$Polygon$translate = F2(
	function (point, poly) {
		return _Utils_update(
			poly,
			{
				origin: A2($author$project$Util$add, poly.origin, point)
			});
	});
var $author$project$Rules$tr = F2(
	function (p, pc) {
		return _Utils_update(
			pc,
			{
				centre: A2($author$project$Util$add, pc.centre, p),
				poly: A2($author$project$Polygon$translate, p, pc.poly)
			});
	});
var $author$project$Rules$step = F2(
	function (tess, bounds) {
		var _v0 = tess.open;
		if (!_v0.b) {
			return tess;
		} else {
			var p = _v0.a;
			var rest = _v0.b;
			if ((!A2($author$project$Util$inside, p.poly.origin, bounds)) || A2(
				$elm$core$List$any,
						...
						return A2(
							...
							function (p2) {
								return A2(
									...
									$author$project$Util$neg(rule.anchor.poly.origin),
									A2(
										$author$project$Rules$tr,
										p.poly.origin,
										A3($author$project$Rules$rt, rule.anchor.poly.origin, p.poly.rotation - rule.anchor.poly.rotation, p2)));
							},
							rule.additions);
					},
					A2(
						...
						function (rule) {
							return A2($author$project$Rules$applies, rule, p);
						},
						tess.rules));
				return _Utils_update(
					tess,
					{
						closed: A2($elm$core$List$cons, p, tess.closed),
						open: _Utils_ap(rest, new_ps)
					});
			}
		}
	});
var $author$project$Rules$fix = F2(
	function (tess, bounds) {
		fix:
		while (true) {
			var _v0 = tess.open;
			if (!_v0.b) {
				return _Utils_update(
					tess,
					{
						closed: $elm$core$List$reverse(tess.closed)
					});
			} else {
				var $temp$tess = A2($author$project$Rules$step, tess, bounds),
					$temp$bounds = bounds;
				tess = $temp$tess;
				bounds = $temp$bounds;
				continue fix;
...
var $author$project$Rules$pt = F2(
	function (alfa, pc) {
		var prevP = A2(
			$author$project$Polygon$getPoint,
			$elm$core$Basics$ceiling(alfa),
			pc.poly);
		var nextP = A2(
			$author$project$Polygon$getPoint,
			$elm$core$Basics$floor(alfa),
			pc.poly);
		return A2(
			...
			A2(
				...
				$elm$core$Basics$ceiling(alfa) - alfa,
				A2($author$project$Util$sub, prevP, nextP)));
	});
var $author$project$Rules$rto = $author$project$Rules$rt(
	{x: 0, y: 0});
var $author$project$Rules$sz = F2(
	function (size, pc) {
		var p = pc.poly;
		return _Utils_update(
			pc,
			{
				centre: A2(
					...
					A2(
						$author$project$Util$mul,
						size,
						A2($author$project$Util$sub, pc.centre, pc.poly.origin))),
				dist: pc.dist * size,
				poly: _Utils_update(
					p,
					{
	...
	var triRule2 = {
		additions: _List_fromArray(
			[
				A2(
				$author$project$Rules$tr,
				A2($author$project$Rules$pt, 5, $author$project$Rules$hexv),
				A2(
					$author$project$Rules$tr,
					A2(
						$author$project$Rules$pt,
						1,
						A2($author$project$Rules$sz, 2, $author$project$Rules$eqi)),
	...
	var hexRule = {
		additions: _List_fromArray(
			[
				...
				A2(
					$author$project$Rules$tr,
					A2($author$project$Rules$pt, 2, $author$project$Rules$hexv),
					A2(
						$author$project$Rules$rto,
						-60,
						_Utils_update(
				...
				A2(
					$author$project$Rules$tr,
					A2($author$project$Rules$pt, 3, $author$project$Rules$hexv),
					A2(
						$author$project$Rules$rto,
						-120,
						_Utils_update(
				...
				A2(
					$author$project$Rules$tr,
					A2($author$project$Rules$pt, 4, $author$project$Rules$hexv),
					A2(
						$author$project$Rules$rto,
						-180,
						_Utils_update(
...
var $author$project$RuleBased$Isogonal$trithagoreanTessellation = function () {
	var triRule3 = {
		additions: _List_fromArray(
			[
				A2($author$project$Rules$sz, 3, $author$project$Rules$eqi),
				A2(
				$author$project$Rules$tr,
				{x: -3, y: 0},
				A2(
			...
			[
				...
				$author$project$Util$neg(
					A2(
						$author$project$Polygon$getPoint,
						1,
						A2($author$project$Polygon$setRotation, 45, $author$project$Rules$squ.poly))),
				A2(
					$author$project$Rules$tr,
					A2(
						$author$project$Polygon$getPoint,
...
var $author$project$Main$tesselations = _Utils_ap($author$project$RuleBased$Regular$regularTesselations, $author$project$RuleBased$Isogonal$isogonalTesselations);
var $author$project$Main$tessDict = $elm$core$Dict$fromList($author$project$Main$tesselations);
...
var $author$project$Polygon$pointSvg = F4(
	function (p, size, origin, rad) {
		return A2(
			$elm$svg$Svg$circle,
			_List_fromArray(
				[
					$elm$svg$Svg$Attributes$cx(
					$elm$core$String$fromFloat(origin.x + (p.x * size))),
					$elm$svg$Svg$Attributes$cy(
					$elm$core$String$fromFloat(origin.y + (p.y * size))),
					$elm$svg$Svg$Attributes$r(
					$elm$core$String$fromFloat(rad))
				]),
			_List_Nil);
	});
...
var $author$project$Main$rulesDisplay = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id('rulesContainer')
			]),
		_List_fromArray(
			[
				A2(
...
var $author$project$Main$tessDisplay = function (model) {
	return A2(
		...
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id('tessContainer')
			]),
		_List_fromArray(
			[
				A2(
		...
		return A2(
			...
			_List_fromArray(
				[
					...
					_Utils_eq(model.selectedTess, tessName) ? 'selected' : ''),
					$elm$html$Html$Events$onClick(
					$author$project$Main$SelectTess(tessName))
				]),
			_List_fromArray(
				[
...
var $author$project$Main$tessOptions = A2($elm$core$List$map, $elm$core$Tuple$first, $author$project$Main$tesselations);
var $author$project$Main$tessMenu = function (model) {
	return A2(
		$elm$html$Html$div,
		_List_fromArray(
			[
				$elm$html$Html$Attributes$id('menuContainer')
			]),
		_List_fromArray(
			[
				A2(
				...
				_List_fromArray(
					[
						$elm$html$Html$Attributes$id('menuContent')
					]),
				A2(
					$elm$core$List$map,
					function (t) {
...
var $author$project$Main$PickQuart = function (a) {
	return {$: 'PickQuart', a: a};
};
						...
						[
							...
							function (value) {
								switch (colorType.$) {
									...
									case 'Quart':
										return $author$project$Main$PickQuart(value);
									default:
...
var $author$project$Main$themeOption = F2(
	function (themeName, model) {
		return A2(
			$elm$html$Html$a,
			_List_fromArray(
				[
					$elm$html$Html$Attributes$class('menuItem'),
					$elm$html$Html$Attributes$class(
					_Utils_eq(model.selectedTheme, themeName) ? 'selected' : ''),
					$elm$html$Html$Events$onClick(
					$author$project$Main$SelectTheme(themeName))
				]),
			_List_fromArray(
				[
					$elm$html$Html$text(themeName)
				]));
	});
		...
		A2(
			...
			_Utils_ap(
				A2(
					$elm$core$List$map,
					function (t) {
						return A2($author$project$Main$themeOption, t, model);
					},
